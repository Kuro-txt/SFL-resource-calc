...
function getItemNameById(id) {
  ...
  // Handle "Item #2988" or "#2988" format
  const hashMatch = String(id).match(/^(?:Item\s*)?#?(\d+)$/i);
  if (hashMatch && KNOWN_ITEMS[hashMatch[1]]) {
    return KNOWN_ITEMS[hashMatch[1]];
  }

  // Handle "collectibles-2988", "wearables-102", etc.
  const prefixMatch = String(id).match(/^[a-z]+-(\d+)$/i);
  if (prefixMatch && KNOWN_ITEMS[prefixMatch[1]]) {
    return KNOWN_ITEMS[prefixMatch[1]];
  }

  // If it's already a clean string name that isn't a raw number or "Item #..."
  if (typeof id === 'string' && isNaN(Number(id)) && !id.startsWith('Item #')) {
    return id;
  }

  // If id is numeric or can be extracted, show Item #<number>
